...
system.time(statsnondefault <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- Statistics2(allvisualdata_all[[i]],aois,"Correct")
...
system.time(newtimetoFixC <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- AllStats(timetoFix[[i]],"ALL")
  x
})
system.time(newtimetoFixTYPC <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- AllStats(timetoFix[[i]],"TYP")
  x
})
system.time(newtimetoFixASDC <- foreach(i = 1:length(conditions)) %dopar%{ 
...
savedataA(newtimetoFixC[[1]],newtimetoFixC[[2]],newtimetoFixC[[3]],newtimetoFixC[[4]],"Stats_CorrectFix3.xls")
savedataA(newtimetoFixTYPC[[1]],newtimetoFixTYPC[[2]],newtimetoFixTYPC[[3]],newtimetoFixTYPC[[4]],"Stats_CorrectTYPFix3.xls")
savedataA(newtimetoFixASDC[[1]],newtimetoFixASDC[[2]],newtimetoFixASDC[[3]],newtimetoFixASDC[[4]],"Stats_CorrectASDFix3.xls")
save(newtimetoFixC,newtimetoFixTYPC,newtimetoFixASDC,file = paste(getwd(),"/Master RDA files_RG/HeatmapdataFixC.rda",sep=""))
