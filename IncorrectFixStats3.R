...
filtervisuals <- function(visuals,aois,fixation){
  
  i <- 1
  newdf <- data.frame(matrix(ncol = 3))
  
  
  xeye <- unlist(visuals[[1]])
  yeye <- unlist(visuals[[2]])
  time <- unlist(visuals[[3]])
  listindex <- vector("integer")
  listcount <- 1
  for (j in 1: length(aois)){
    betweeners <- dplyr::between(as.numeric(xeye),as.numeric(aois[[j]][1]),as.numeric(aois[[j]][2]))
    betweenersy <- dplyr::between(as.numeric(yeye),as.numeric(aois[[j]][3]), as.numeric(aois[[j]][4]))
    betweentime <-  dplyr::between(as.numeric(time),-1,-1)
    trueindex <- intersect(intersect(grep(TRUE,betweenersy),grep(TRUE, betweeners)),grep(FALSE,betweentime))
    
    
    
    consecutiveCount <- 1
    i <- 1
    while(i<(length(trueindex)-1)){
      
      if(abs((trueindex[i]-trueindex[i+1])) == 1){
        consecutiveCount <- consecutiveCount + 1
      } else{
        consecutiveCount <- 1
      }
      
      if (consecutiveCount == fixation){
        #browser()
        listindex[listcount] <- trueindex[i]
  ...
  newdf <- visuals[listindex,]
  # while (i < nrow(visuals)){
  #   # if(dplyr::between(as.numeric(unlist(visuals[[1]])),as.numeric(aois[[1]][1]),as.numeric(aois[[1]][2]))){
  #   if(dplyr::between(as.numeric(xeye[i]),as.numeric(aois[[1]][1]),as.numeric(aois[[1]][2]))){
  #     consecutive <- FALSE
  #     browser()
  #     for (j in i:(i+fixation-1)){
  #       if(dplyr::between(xeye[i],as.numeric(aois[[1]][1]),as.numeric(aois[[1]][2])) & (j < (i+fixation-1))){
  #         consecutive <- TRUE
  #         visuals[[1]][j] <- NA
  #         visuals[[2]][j] <- NA
  #         visuals[[3]][j] <- NA
...
visualdata_gen <- function(dataframe,aois,fixation){
  ...
  for (i in 1: length(participants)){
    ...
    if( !is.null(nrow(pardata))){
      ...
      for (j in seq(1,ncol(visualdata),3)){
        newdf <- as.data.frame(filtervisuals(visualdata[,(j:(j+2))],aois,fixation))
...
system.time(allvisualdata_all <- foreach (i = 1:length(conditions)) %dopar%{
  x <- visualdata_gen(conditions[[i]],aois,5)
  x
})
...
system.time(statsnondefault <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- Statistics2(allvisualdata_all[[i]],aois,"Incorrect")
...
#All, TYP, ASD Summaries
system.time(newtimetoFixI <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- AllStats(timetoFix[[i]],"ALL")
  x
})
system.time(newtimetoFixTYPI <- foreach(i = 1:length(conditions)) %dopar%{ 
  x <- AllStats(timetoFix[[i]],"TYP")
  x
})
system.time(newtimetoFixASDI <- foreach(i = 1:length(conditions)) %dopar%{ 
...
savedataA(newtimetoFixI[[1]],newtimetoFixI[[2]],newtimetoFixI[[3]],newtimetoFixI[[4]],"Stats_FixIncorrect3.xls")
savedataA(newtimetoFixTYPI[[1]],newtimetoFixTYPI[[2]],newtimetoFixTYPI[[3]],newtimetoFixTYPI[[4]],"Stats_FixIncorrectTYP3.xls")
savedataA(newtimetoFixASDI[[1]],newtimetoFixASDI[[2]],newtimetoFixASDI[[3]],newtimetoFixASDI[[4]],"Stats_FixIncorrectASD3.xls")
save(newtimetoFixI,newtimetoFixTYPI,newtimetoFixASDI,file = paste(getwd(),"/Master RDA files_RG/HeatmapdataFixI.rda",sep=""))
